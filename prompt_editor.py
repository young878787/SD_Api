...
輸入現有 Prompt 與修改想法後，AI 會精準替換相關標籤並保留其餘內容。
"""
import os
import re
import sys
...
_log_file_handle = None  # 目前 session 的 log 檔句柄
_log_setup_pid = None    # 已初始化的 PID，避免重複寫入
...
class _TeeStream:
    ...
    def fileno(self):
        # Gradio 有時會呼叫 fileno，直接回傳原始終端 fd
        return self._primary.fileno()


def setup_session_log(log_dir: str) -> str:
    """
    初始化本次 session 的 log 檔案。

    同時攔截 sys.stdout / sys.stderr，讓 print 也能寫入 log。
    ...
    # Gradio 啟動時可能多執行緒同時 import，先用 PID 做去重。
...
- 同一類或相近意思的描述不要反覆堆疊太多，優先保留最精準的 1~2 種變體就好，避免出現同樣意思的重複標籤
...
class PromptEditor:
    ...
    def __init__(self, sd_url="http://127.0.0.1:7860", output_dir="outputs"):
        ...
        if active:
            unique_labels = list(dict.fromkeys(p[3].rsplit(" ", 1)[0] for p in active))
    ...
    # default_base: base_env 未設定時的預設值
    # model_env   : 主模型的環境變數名
    # backup_env  : 備用模型清單的環境變數名
    ...
    _PROVIDER_CONFIGS: dict = {
        ...
        "opencode_go": {
            ...
            "model_env":    "OPENCODE_GO_MODEL_NAME",
            "backup_env":   "OPENCODE_GO_BACKUP_MODELS",
            "timeout_env":  "OPENCODE_GO_API_TIMEOUT",
            "label":        "OpenCode Go",
        },
    }
    ...
    def _build_provider_list(self) -> list:
        """
        建立 AI provider 清單，格式為
        [(client, timeout, model_name, label), ...]。

        依 .env 的 AI_PROVIDERS 決定優先順序，只保留有填 API Key 的提供商。
        每個提供商可同時提供主模型與備用模型；失敗時會依序切換。
        """
        if self._providers_cache is not None:
            return self._providers_cache

        # 讀取啟用清單，預設全開以維持向後相容
        enabled_str = os.getenv("AI_PROVIDERS", "nvidia_api,open_router,gemini,qwen,opencode_go").strip()
    ...
    def edit_prompt_with_ai(self, original_prompt: str, user_idea: str,
        ...
        if not providers:
            ...
            print("   1. .env 的 AI_PROVIDERS 是否包含至少一個提供商（nvidia_api / open_router / gemini / qwen / opencode_go）")
            print("   2. 對應的 API Key 是否已填入（如 NVIDIA_API_KEY / OPENROUTER_API_KEY / GEMINI_API_KEY / DASHSCOPE_API_KEY / OPENCODE_GO_API_KEY）")
            return None, None
...
[/FINAL_PROMPT]"""
        ...
        for client, timeout, model_name, provider_label in providers:
            ...
            try:
                ...
                # content 可能是 None；某些思考型模型只回 reasoning_content
                raw_content = message.content
                if raw_content is None:
                    # 再嘗試從 reasoning_content 取值
                ...
                if extracted:
                    ...
                    # 3. 最後用智慧 fallback 找出最像 SD prompt 的段落
    ...
    def run_attempt_pipeline(
        ...
        """
        單次嘗試的完整流程（thread-safe）：AI 修改後立即送 SD 生圖。

        設計成可被 ThreadPoolExecutor 並行呼叫，各嘗試彼此獨立。
        ...
        if gen_result and "images" in gen_result:
            metadata = self._build_complete_metadata(modified_prompt, config, ai_metadata, original_prompt, user_idea)
    ...
    def _build_complete_metadata(self, modified_prompt: str, config: dict,
                                  ...
                                  original_prompt: str = "",
                                  user_idea: str = "") -> dict:
        """把 AI 結果與 SD config 組成圖片存檔用的 metadata。"""
        return {
    ...
    def save_images(self, response: dict, metadata: dict = None) -> list:
        """
        將生成圖片存到本次 session 資料夾，並同步輸出對應的 .txt 記錄。
    ...
    def _save_image_metadata(self, txt_filepath: str, metadata: dict, image_filename: str):
        ...
        try:
            ...
            if metadata.get("original_prompt"):
                lines.append("👤 用戶原始 Prompt")
                lines.append("-" * 80)
                lines.append(f"   {metadata['original_prompt']}")
                lines.append("")
            if metadata.get("user_idea"):
                lines.append("💡 用戶修改想法 (Idea)")
                lines.append("-" * 80)
                lines.append(f"   {metadata['user_idea']}")
                lines.append("")
...
def main():
    ...
    while True:
        ...
        print()
        # 保持 current_prompt 不變，讓每輪都基於同一基底修改
